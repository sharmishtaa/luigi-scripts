...
class register(luigi.Task):
	...
	def requires(self):
		...
		inputtilespec = "%s/processed/stitched_tilespec_ff/DAPI_%01d_rib%04dsess%04dsect%04d.json"%(self.info['rootdir'],self.session,self.ribbon, self.session, self.section)	
		...
		referencetilespec = "%s/processed/stitched_tilespec_ff/DAPI_%01d_rib%04dsess%04dsect%04d.json"%(self.info['rootdir'],self.refsession,self.ribbon, self.refsession, self.section)
		...
		self.cmd = "java -cp /data/array_tomography/ForSharmi/sharmirender/render/render-app/target/render-app-0.3.0-SNAPSHOT-jar-with-dependencies.jar:/pipeline/stitching/Fiji_Plugins-2.0.1.jar org.janelia.alignment.RegisterSections --modelType 1 --percentSaturated 0.9f --maxEpsilon 2.5 --initialSigma 2.5 --inputtilespec "
